...
//	timeSeparators : [ ' ', ':', 'AM', 'PM' ], // An array of time separators:
	...
	formatDate : function(day, month, year, hour, minute) {
		...
		if (this.timeSeparators && this.timeSeparators.length > 1
			...
			dateString += this.timeSeparators[0] + ((hour >= 10) ? hour : ('0' + hour))
	...
	parseDate : function(Text) {
		...
		if (currentDate.length == 3) {
			currentDay = Number(this.trimNumber(currentDate[this.dateFormat
					.indexOf('D')]));
			currentMonth = Number(this.trimNumber(currentDate[this.dateFormat
					.indexOf('M')])) - 1;
			currentYear = Number(this.trimNumber(currentDate[this.dateFormat
					.indexOf('Y')]));
		} else {
		...
		if (this.timeSeparators && this.timeSeparators.length > 1
			...
			if (currentTime != null && currentTime.length == 2) {
				currentHour = Number(this.trimNumber(currentTime[0]));
				...
				if (this.timeSeparators.length > 2) {
					if (currentTime[1].indexOf(this.timeSeparators[2]) == currentTime[1].length
						...
						if (currentHour == 12)
							currentHour = 0;
						currentMinute = Number(this.trimNumber(currentTime[1]
	...
	showCalendar : function() {
		...
		if (this.timeSeparators.length > 1) {
			...
			if (this.timeSeparators.length == 4) {
				...
				for ( var a = 0; a < 24; a++) {
					if (this.selectedHour == a)
						timeSelect += '<option value="' + a
								+ '" selected="selected">' + ((a >= 10) ? a : ('0' + a)) + '</option>';
					else
						timeSelect += '<option value="' + a + '">' + ((a >= 10) ? a : ('0' + a))
								+ '</option>';
				}
			}
		...
		if (!this.changeYear) {
			...
			if (years.length != 2) {
				...
				year = this.selectedYear + Number(years[0]);
				endYear = this.selectedYear + Number(years[1]);
			} else {
				year = Number(years[0]);
	...
	checkExternalClick : function(event) {
		if (popUpCal.popUpShowing) {
			...
			while (node && node != cal && node.className != popUpCal.input[0].className && node.className != 'calendar_trigger') {
